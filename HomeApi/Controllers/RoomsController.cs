    ...
    public class RoomsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateRoom([FromRoute] Guid id, [FromBody] EditRoomRequest request)
        {
            ...
            await _repository.UpdateRoom(room, new UpdateRoomQuery(request.NewName,
                request.NewArea, request.NewGasConnected, request.NewVoltage));
            return StatusCode(200, $"Комната обновлена. Имя - {room.Name}, Этаж - {room.Area},Газ - {room.GasConnected},Электричество {room.Voltage}.");
        }
    }
}